rankhospital<- function(state,outcome,num="best"){
  ...
  if(num=="best"){
    result<-as.character(my_state_data[my_state_data$dat_rank==1,"hospital_name"])
  }
  else if(num=="worst")
  {
    n<-nrow(my_state_data)
    result<-as.character(my_state_data[my_state_data$dat_rank==n,"hospital_name"])
  }
  else
  {
    result<-as.character(my_state_data[my_state_data$dat_rank==num,"hospital_name"])
  }
  return(result)
}