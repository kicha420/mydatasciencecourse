rankall<- function(outcome,num="best"){
  ...
  if(num=="best"){
    result_hos1<-as.character(my_state_data[my_state_data$dat_rank==1,"hospital_name"])
    result_state1 <- state
  }
  else if(num=="worst")
  {
    n<-nrow(my_state_data)
    result_hos1<-as.character(my_state_data[my_state_data$dat_rank==n,"hospital_name"])
    result_state1 <- state
  }
  else
  {
    result_hos1<-as.character(my_state_data[my_state_data$dat_rank==num,"hospital_name"])
    if(length(result_hos1)==0){
      result_hos1<-NA
    }
    result_state1<- state
  }
  result_hos<-rbind(result_hos,result_hos1)
  result_state<-rbind(result_state,result_state1)
  }
  result<-cbind(result_hos,result_state)
  row.names(result)<-NULL
  result<- data.frame(result)
  colnames(result)<-c("hospital_name","state")
  #my_result_order <- order(result[,"state"],decreasing = FALSE)
